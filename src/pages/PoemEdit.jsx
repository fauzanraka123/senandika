...
import { getCategories, savePoem, getPoemBySlug } from '../mockDb';
...
export default function PoemEdit() {
  ...
  useEffect(() => {
    ...
    if (!loadedPoem) { navigate('/dashboard'); return; }
    setPoem(loadedPoem);
    setTitle(loadedPoem.title);
    setCategoryId(loadedPoem.category_id.toString());
    setExcerpt(loadedPoem.excerpt || '');
    setContent(loadedPoem.content);
    setStatus(loadedPoem.status);
  }, [slug, navigate]);

  useEffect(() => {
    if (poem && editorRef.current && !quillInstance.current) {
      ...
      quillInstance.current.root.innerHTML = poem.content;
  ...
  const handleSave = (e) => {
    ...
    const saved = savePoem({ ...poem, title, category_id: categoryId, excerpt, content, status });
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-1/2 space-y-8">
            <div className="bg-white dark:bg-[#1A1A1A] rounded-[2rem] p-8 shadow-sm border border-stone-100 dark:border-stone-800">
              <h2 className="text-2xl font-serif font-bold text-stone-900 dark:text-stone-100 mb-6">Edit Puisi</h2>
            ...
            <div className="bg-white dark:bg-[#1A1A1A] rounded-[2rem] p-8 md:p-12 shadow-sm border border-stone-100 dark:border-stone-800 sticky top-28">
              ...
              <div className="text-center mb-10 pb-8 border-b border-stone-100 dark:border-stone-800">
                ...
                <p className="text-stone-400 dark:text-stone-600 text-xs font-bold uppercase tracking-widest mt-2">
                  Oleh Anda &bull; Terakhir Diperbarui
                </p>
              </div>
