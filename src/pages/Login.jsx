...
export default function Login() {
  ...
  const handleGoogleLogin = () => {
    // Standard mock user for Google Login
    const user = loginUser('fauzan@RuangKata.com', 'password');
    if (user) {
      window.dispatchEvent(new Event('rk-user-changed'));
      navigate('/dashboard');
    }
  ...
  return (
    <div className="min-h-[80vh] flex flex-col justify-center items-center px-4">
      <div className="w-full max-w-md p-8 bg-white dark:bg-[#1A1A1A] rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800">
        ...
        <form onSubmit={handleLogin} className="space-y-4 mb-6">
          ...
          <button
            type="submit"
            className="w-full py-3 bg-[#8B5E3C] dark:bg-[#C9A27C] text-white dark:text-[#1A1A1A] rounded-xl font-bold text-sm hover:opacity-90 transition-opacity uppercase tracking-widest cursor-pointer"
          >
            Masuk / Daftar
          </button>
        </form>
